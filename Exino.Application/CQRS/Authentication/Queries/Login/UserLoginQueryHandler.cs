...
namespace Exino.Application.CQRS.Authentication.Queries.Login
{
    ...
    public class UserLoginQueryHandler : IRequestHandler<UserLoginQueryRequest, IResult<UserLoginQueryResponse>>
    {
        ...
        public async Task<IResult<UserLoginQueryResponse>> Handle(UserLoginQueryRequest request, CancellationToken cancellationToken)
        {
            var user = await _identityService.AuthenticateUser(request.Email, request.Password);

            if (user == null)
            {
                return Response<UserLoginQueryResponse>.ErrorResponse(new string[] { $"Invalid Credentials for '{request.Email}'." });
            }
